...
function updateScrollLeft() {
    scrollLeftPosition.value = mediaContainer.value.scrollLeft
    isScrolledToRight.value = mediaContainer.value.scrollLeft + mediaContainer.value.clientWidth >= mediaContainer.value.scrollWidth - 1 // -1 voorkomt afrondings problemen bij browsers
}

onMounted(() => {
    mediaContainer.value.addEventListener('scroll', updateScrollLeft)
    updateScrollLeft(mediaContainer.value.scrollLeft) // initialize
})
onBeforeUnmount(() => {
    mediaContainer.value.removeEventListener('scroll', updateScrollLeft)
})
</script>
<template>
    <div class="relative">
        ...
        <div ref="mediaContainer" class="flex gap-2 overflow-y-hidden overflow-x-scroll lg:overflow-x-hidden pb-2">
            <slot />
        </div>
    </div>
</template>
